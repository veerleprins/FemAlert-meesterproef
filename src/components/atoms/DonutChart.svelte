<script>
  ...
  let content = tooltipValue > 1 ? 'meldingen' : 'melding'

  const showTooltip = (id, color, name, value) => {
    ;(shownTooltip = id), (tooltipColor = color)
    tooltipName = name
    tooltipValue = value
    content = tooltipValue > 1 ? 'meldingen' : 'melding'
  }
</script>
    ...
    <g transform="translate(75,75)">
      {#each arcs as arc}
        ...
        <path
          d={arc.d}
          fill={arc.color}
          on:mouseenter={showTooltip(arc.id, arc.color, arc.label, arc.value)}
        />
      {/each}
    </g>
  </svg>
